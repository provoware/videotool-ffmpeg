...
def run_runner(settings_path: Path, rules_path: Path) -> Path:
    ...
    cmd = [str(Path(__file__).with_name("automation_runner.py")), "--settings", str(settings_path), "--rules", str(rules_path)]
...
def prepare_watch(folder: Path):
    ...
    assets = root()/"assets"/"default_assets"
    # Copy one image
    shutil.copy(assets/"test_image.jpg", folder/"test_image.jpg")
    # Copy audio (wav)
    shutil.copy(assets/"test_audio_10s.wav", folder/"test_audio_10s.wav")

def make_overrides(base_data_dir: Path, watch_folder: Path, settings_base: dict, rules_base: dict, target_bitrate: int, min_bitrate: int) -> tuple[Path, Path]:
...
def main():
    ...
    settings_base = load_json(cfg_dir()/"settings.json", {})
    rules_base = load_json(cfg_dir()/"automation_rules.json", {})

    # sandbox base dir
    sandbox = user_data()/"selftest"
    base_ok = sandbox/"ok"
    ...
    s_ok, r_ok = make_overrides(base_ok, watch_ok, settings_base, rules_base, target_bitrate=320, min_bitrate=192)
    # B) BAD: target 96, min 192 -> should fail validation and create quarantine job
    s_bad, r_bad = make_overrides(base_bad, watch_bad, settings_base, rules_base, target_bitrate=96, min_bitrate=192)
    ...
    main_reports = user_data()/"reports"
    ...
    qjobs = base_bad/settings_base["paths"]["quarantine_jobs_dir"]/f"quarantine_jobs_{today}.json"
    ...
    summary = {
        ...
        "reports": {
            "ok": str(rep_ok_copy),
            "quarantine": str(rep_bad_copy)
        },
        "sandbox": {
            "ok": str(base_ok),
            "quarantine": str(base_bad)
        }
    }
