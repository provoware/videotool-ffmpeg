#!/usr/bin/env python3
import json, subprocess, shutil, time, re, argparse
...
def default_cfg_dir() -> Path:
    return root()/"portable_data"/"config"

def default_settings_path() -> Path:
    return default_cfg_dir()/"settings.json"

def default_rules_path() -> Path:
    return default_cfg_dir()/"automation_rules.json"
...
def build_output_name(audio_path: Path, preset: str, sw: bool, idx: int, settings: dict) -> str:
    ...
    nummer = f"{idx:03d}"
    sws = "_sw" if sw else ""
    name = tmpl.format(audio=audio, vorlage=safe_slug(preset), datum=dt, uhrzeit=tm, nummer=nummer, sw=sws)
    return safe_slug(name) + ".mp4"

def ensure_structure(base: Path, settings):
    for rel in [
        settings["paths"]["exports_dir"], settings["paths"]["library_audio_dir"], settings["paths"]["library_images_dir"],
        settings["paths"]["quarantine_dir"], settings["paths"]["quarantine_jobs_dir"], settings["paths"]["reports_dir"],
        settings["paths"]["trash_dir"], settings["paths"]["staging_dir"]
    ]:
...
def load_or_create_quarantine_list(base: Path, qjobs_rel: str, day: str) -> Path:
    ...
    if not qfile.exists():
        doc = {
            ...
            "list_status": "offen",
            "closed_at": None,
            "summary": {"total":0,"done":0,"postponed":0,"hard_cases":0,"open":0},
            "items": []
        }
        save_json(qfile, doc)
    return qfile
...
def update_list_status(doc: dict) -> dict:
    ...
    postponed = sum(1 for it in doc.get("items", []) if it.get("status") == "zurueckgestellt")
    hard = sum(1 for it in doc.get("items", []) if it.get("status") == "fest")
    openish = sum(1 for it in doc.get("items", []) if it.get("status") in ("bereit","laeuft"))
...
def append_quarantine_job(qfile: Path, run_id: str, nr: int, preset: str, output_quarantine: Path, staging_audio: Path, staging_image: Path|None, reason: str, validation: dict):
...
def run(settings_path: Path, rules_path: Path) -> Path:
    ...
    if not watch.exists():
        log_line(logs_dir, f"Automatik abgebrochen: Watchfolder fehlt: {watch}")
        raise SystemExit(1)
    ...
    if lock.exists():
        log_line(logs_dir, "Lock vorhanden: Automatik läuft schon. Abbruch.")
        raise SystemExit(0)
    ...
    try:
        audio_exts = set([e.lower() for e in rules.get("scan", {}).get("audio_extensions", [".mp3",".wav",".flac",".m4a"])])
        img_exts = set([e.lower() for e in rules.get("scan", {}).get("image_extensions", [".jpg",".jpeg",".png",".webp"])])
        audios = [p for p in watch.iterdir() if p.is_file() and p.suffix.lower() in audio_exts]
        images = [p for p in watch.iterdir() if p.is_file() and p.suffix.lower() in img_exts]

        audios.sort(key=lambda p: p.stat().st_mtime)
        images.sort(key=lambda p: p.stat().st_mtime)

        fallback_img = (root()/rules.get("pairing", {}).get("fallback_image","assets/default_assets/preset_cover.jpg")).resolve()
        preset_id = rules.get("presets", {}).get("default_preset_id", "youtube_hd_ton_safe")
        ...
        exports_day = base/settings["paths"]["exports_dir"]/day; exports_day.mkdir(parents=True, exist_ok=True)
        quarantine_day = base/settings["paths"]["quarantine_dir"]/day; quarantine_day.mkdir(parents=True, exist_ok=True)
        lib_a = base/settings["paths"]["library_audio_dir"]/day; lib_i = base/settings["paths"]["library_images_dir"]/day
        lib_a.mkdir(parents=True, exist_ok=True); lib_i.mkdir(parents=True, exist_ok=True)

        qfile = load_or_create_quarantine_list(base, settings["paths"]["quarantine_jobs_dir"], day)

        staged_images = []
        for img in images:
            if not stable_file(img):
                report["repairs"].append({"type":"skip_unstable_image","file":str(img)})
                continue
            dst = staging_day/"images"/(safe_slug(img.stem) + f"_in_work_{run_id}" + img.suffix.lower())
            try:
                shutil.move(str(img), str(dst))
                staged_images.append(dst)
            except Exception as e:
                report["repairs"].append({"type":"move_fail_image","file":str(img),"error":str(e)})
        ...
        for idx, aud in enumerate(audios, start=1):
            ...
            if not stable_file(aud):
                job.update({"status":"uebersprungen","reason":"download_nicht_stabil","audio":str(aud)})
                report["jobs"].append(job)
                continue

            aud_dst = staging_day/"audio"/(safe_slug(aud.stem) + f"_in_work_{run_id}" + aud.suffix.lower())
            try:
                shutil.move(str(aud), str(aud_dst))
            except Exception as e:
                job.update({"status":"quarantaene","reason":"move_fail_audio","audio":str(aud),"error":str(e)})
                report["jobs"].append(job); qn += 1
                continue

            img_use = staged_images[idx-1] if (idx-1) < len(staged_images) else fallback_img

            out_name = build_output_name(aud_dst, preset_id, False, idx, settings)
            out_tmp = (root()/"portable_data"/"cache"/"temp_renders"/out_name)
            ...
            cmd += [
                "-loop","1","-i", str(img_use),
                "-i", str(aud_dst),
                "-vf", f"scale={W}:{H}:force_original_aspect_ratio=increase,crop={W}:{H}",
                "-c:v","libx264","-tune","stillimage","-preset","medium","-crf","19",
                "-pix_fmt","yuv420p","-r", str(FPS),
                "-c:a","aac","-b:a", f'{settings["audio"]["target_bitrate_kbps"]}k',
                "-ar", str(settings["audio"]["target_samplerate_hz"]),
                "-shortest","-movflags","+faststart",
                str(out_tmp)
            ]

            job.update({"audio":str(aud_dst),"image":str(img_use),"output_tmp":str(out_tmp)})
            try:
                subprocess.check_call(cmd)
            except Exception as e:
                qn += 1
                job.update({"status":"quarantaene","reason":"ffmpeg_fail","error":str(e)})
                ...
                marker.write_text("FFmpeg Fehler beim Export. Siehe Report.\n", encoding="utf-8")
                append_quarantine_job(qfile, run_id, idx, preset_id, marker, aud_dst, img_use if img_use!=fallback_img else None, "ffmpeg_fail", {})
                report["jobs"].append(job)
                continue
            ...
            ok_audio = False
            bitrate_kbps = None; samplerate = None
            validation = {}
            try:
                ...
                ok_bitrate = (bitrate_kbps is not None and bitrate_kbps >= min_br) if min_br > 0 else True
                ok_sr = (samplerate == int(settings["audio"]["target_samplerate_hz"]))
                ok_audio = ok_bitrate and ok_sr
                validation = {"audio_bitrate_kbps": bitrate_kbps, "audio_samplerate_hz": samplerate, "ok": bool(ok_audio)}
            ...
            if not ok_audio:
                ...
                job.update({"status":"quarantaene","reason":"audio_check_fail","output_quarantine":str(q_out)})
                append_quarantine_job(qfile, run_id, idx, preset_id, q_out, aud_dst, img_use if img_use!=fallback_img else None, "audio_check_fail", validation)
                report["jobs"].append(job)
                continue
            ...
            aud_final = lib_a/(safe_slug(aud_dst.stem) + f"_used_{used_ts}" + aud_dst.suffix.lower())
            shutil.move(str(aud_dst), str(aud_final))

            img_final = ""
            if img_use != fallback_img and Path(img_use).exists():
                img_p = Path(img_use)
                img_final_path = lib_i/(safe_slug(img_p.stem) + f"_used_{used_ts}" + img_p.suffix.lower())
                shutil.move(str(img_p), str(img_final_path))
                img_final = str(img_final_path)

            job.update({"status":"fertig","output_final":str(out_final),"inputs_final":{"audio":str(aud_final),"image":img_final}})
            report["jobs"].append(job)
            ok += 1
        ...
        log_line(logs_dir, f"Automatik Lauf {run_id}: fertig={ok} quarantaene={qn} gesamt={len(audios)}")
        return report_path
