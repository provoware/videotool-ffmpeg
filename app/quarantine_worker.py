...
def run(job_id: str|None = None) -> int:
    ...
    qjobs = base/settings["paths"]["quarantine_jobs_dir"]/f"quarantine_jobs_{today}.json"
    doc = load_json(qjobs, default={"schema_version":1,"date":today,"items":[],"list_status":"offen","closed_at":None})
    ...
    image_target = Path(job["paths"].get("staging_image","")) if job.get("paths") else Path()
    ...
    if not image_target.exists():
        ...
        fallback = root()/"assets"/"default_assets"/"preset_cover.jpg"
    ...
    preset_id = job.get("preset") or "youtube_hd_ton_safe"
    out_name = safe_slug(Path(job.get("output_file","")).stem or f"rework_{job['job_id']}") + ".mp4"
    out_tmp = cache_dir()/"temp_renders"/out_name
    out_final = exports_day/out_name

    cmd = [
        "ffmpeg","-hide_banner","-y",
        "-loop","1","-i",str(image_target),
        "-i",str(audio_target),
    ...
    try:
        ...
        job["status"] = "bereit" if job["tries"] < int(job.get("max_tries",3)) else "fest"
    ...
    try:
        ...
        ok_audio = (samplerate == settings["audio"]["target_samplerate_hz"]) and (bitrate_kbps is None or bitrate_kbps >= settings["audio"]["min_bitrate_kbps"])
        job["validation"] = {"audio_bitrate_kbps": bitrate_kbps, "audio_samplerate_hz": samplerate, "ok": bool(ok_audio)}
    except Exception as e:
